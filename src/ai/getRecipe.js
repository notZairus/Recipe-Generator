...
export async function getRecipe(ingredients) {
  ...
  const response = await hf.chatCompletion({
    ...
    messages: [
        { role: "system", content: "You are an assistant that receives a list of ingredients that a user has and suggests a recipe they could make with some or all of those ingredients. You don't need to use every ingredient they mention in your recipe. The recipe can include additional ingredients they didn't mention, but try not to include too many extra ingredients. FORMAT IT ON json i just need to see a json response pls, i dont need an explanation i just need the json string so that i can easily extract the ingredient and instruction" },
